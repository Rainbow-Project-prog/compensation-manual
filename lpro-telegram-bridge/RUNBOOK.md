...
- **自分側（L-Pro 側）の発言はサイレントで流す（`MIRROR_SELF=silent`・2026-09-11〜）**: 一斉配信・自動応答・PC直返信は
  「🔷 自分(L-Pro): 」付きでトピックに入るが、Telegram のサイレント送信（音・バイブなし）にする。顧客の発言を含む
  メッセージだけが通常通知で鳴るので、「顧客からの返信」と「ステップ配信が流れただけ」を通知で区別できる。
  `MIRROR_SELF=notify` で従来どおり通知付き、`off` で自分側を流さない（ブリッジが読んだ分は既読台帳に記録するので、
  後で戻してもその分が一斉に流れることはない。off の間に一度も読まれなかった顧客の履歴窓は、戻した後の初回接触で
  通常どおり末尾 `BOOTSTRAP_TAIL` 件だけ配られる）。切り替えは案件の `.env` を変えて再起動。
  注: サイレント送信でも未読バッジは付く（Telegram の仕様）。鳴らないだけ。
