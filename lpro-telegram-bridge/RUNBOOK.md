...
| 3. Lpro のログイン | プロファイルの有無・`.lpro-session`（ログイン確認OKのたびに更新）の鮮度 | **C章**。ログイン切れは pm2 restart では直らない（人が画面で入り直す） |
...
| `⚠️ Lpro に未ログインです…手動ログイン待機中` | セッション失効（C章）。赤いバナーのブラウザでログインする。未ログインが 20 秒続いてから初回、以後 15 分ごと（20 秒以内に自己回復した空振りでは飛ばない） |
...
- **⚠️ Lpro セッションの失効（無人運用の最大の制約）**: Lpro のログインセッションは、ブリッジが巡回している限り
  ...
  ブリッジは失効を検知すると poll エラー経路から「未ログイン」状態に入り、headed ブラウザで**手動ログイン待ち**になる
  （巡回はその間停止する。C章参照。Telegram に `⚠️ Lpro に未ログインです` が届く）。
