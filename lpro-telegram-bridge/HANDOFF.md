...
```
スマホ（Telegramアプリ）
   ├─ グループA：チャット応対（自動応答なし・全部手動）      ┐ 顧客1人=1トピック
   └─ グループB：ダイレクトトーク応対（自動応答の取りこぼしを拾う）┘ 受信箱ごとに別グループ
   ⇅  送受信（返信は必ず元の受信箱・元のグループへ戻る）
ブリッジ常駐（Node + TypeScript）  ← あなたのPCで常時起動
   ...
   ├─ db.ts        : SQLite（顧客↔トピック↔既読ハッシュ台帳。顧客キーは「受信箱ID:会員ID」）
   ├─ queue.ts     : Playwright操作を直列化（巡回と送信の衝突防止）
   └─ index.ts     : 巡回ループ＋配線
   ⇅  ログイン状態を永続化して自動操作
Lpro（2つの受信箱: チャット応対 / ダイレクトトーク応対。どちらもブラウザ管理画面）
```

- **新着検知**：有効な**受信箱ごとに**顧客行（`tr.rowitem`）を巡回 → 行内の会話履歴を読み → 会員ID+日時+本文のハッシュを台帳（`seen_messages`）と照合し、未知分だけ**その受信箱に対応する Telegram グループ**へ。
- **返信反映**：Telegram のトピックで打つ → グループ＋スレッドIDから受信箱と顧客を特定 → Lpro の元の受信箱の該当会話に自動入力＆送信。
- **取り違え防止**：顧客ごとに専用トピック＋受信箱ごとに別グループ。顧客キーは受信箱込みの `受信箱ID:会員ID`。
...
1. Telegram で **@BotFather** → `/newbot` → トークン取得（`TELEGRAM_BOT_TOKEN`）。bot は1つでよい。
2. **受信箱ごとにスーパーグループを作成**する（チャット応対用・ダイレクトトーク応対用）。使う受信箱の分だけでよい（最低1つ）。各グループの設定で **「トピック（Topics）」を ON**。
3. **同じ bot を両方のグループに**それぞれ**管理者**として追加し、**「トピックの管理（Manage Topics）」権限を付与**。
4. 各グループの `chat_id` を後述の `npm run chatid` で取得（`-100...` の数値）。チャット応対グループを `CHAT_GROUP_CHAT_ID`、ダイレクトトーク応対グループを `TALK_GROUP_CHAT_ID` に記入。
...
#    （各受信箱は URL とグループID の両方が揃うと有効。最低1つ）
...
npm run chatid
#    各グループの -100... を取得し、チャット応対を CHAT_GROUP_CHAT_ID、
#    ダイレクトトーク応対を TALK_GROUP_CHAT_ID に記入
...
- **Lpro には2つの受信箱がある**：①チャット応対（`chat_message`、自動応答なし＝全部手動なので必ず監視）
  ②ダイレクトトーク応対（`linechat_message`、基本は自動応答だがキーワードに変な絵文字が付くと不発になり
  取りこぼす——それを拾う目的で監視）。両者は別グループへ配信し、有効化は受信箱ごとに
  `CHAT_TALK_URL`+`CHAT_GROUP_CHAT_ID` / `TALK_TALK_URL`+`TALK_GROUP_CHAT_ID` の両方が揃うかで決まる
  （少なくとも1つ有効にすること。片方だけの設定は doctor がエラーにする）。
- **新着検知はフィンガープリント方式**（`seen_messages` 台帳）で実装済み。同数入れ替わり
  （1件消えて1件増える）・履歴窓のズレでも取りこぼし／再配信は起きない
  （`test/logic.test.ts` に回帰テスト有）。
- 顧客キーは内部的に **`受信箱ID:会員ID`**（会員ID部分は `SELECTORS.memberIdText` の完全一致、
  アカバンでも不変）を使う。受信箱を前置することで、同じ会員IDが両受信箱に現れても別トピックとして
  正しく分離される。顧客名や行全文をキーにしてはならない（同名衝突・プレビュー変化でキーが揺れ、
  顧客1人に複数トピックができる）。
