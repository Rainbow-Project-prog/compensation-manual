...
- **トピックは配信時に作成**（空トピックを量産しない）。閉じられていたら自動で開き直し、
  削除されていたら次回配信時に作り直す。
- **自分側（L-Pro 側）の発言の扱い `MIRROR_SELF`**: 既定 `silent` ＝ 一斉配信・自動応答・PC直返信はトピック内の
  「🔷 L-Pro側（配信・返信）」メッセージへ**編集で追記**（編集は未読・通知を発生させない）。鳴る・バッジが増えるのは
  顧客の発言だけで、会話の流れはトピック内で時系列に読める。`notify` で通知付きの通常メッセージ、`off` で流さない（RUNBOOK D章）。
