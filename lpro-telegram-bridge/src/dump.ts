/**
 * 実DOM収集ツール（SELECTORS 確定用の診断。`npm run dump`）。
 * 引数なし: ブラウザが開くので Lpro にログインし、トーク応対（ダイレクトトーク）画面を表示するだけでよい。
 *          トーク画面の要素（.btn_send 等）をどこかのフレームで検知したら、全フレームの HTML を dump/ に保存して終了する。
 * URL 指定: `npm run dump -- <URL> <サブフォルダ>` でその画面を開いて 10 秒後に保存する（ログイン画面など。RUNBOOK C）。
 * ※ 本体と同じブラウザプロファイルを使うので、稼働中のブリッジは先に止めること（pm2 stop）。
